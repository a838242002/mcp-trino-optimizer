...
NOTE: Plan 03 and 04 are both Wave 2. Plan 04 depends on Plan 01 + 02 outputs (ports, classifier, errors, auth, settings). It also uses TrinoClient from Plan 03, but the live sources are thin wrappers — the executor should create `client.py` from Plan 03 first, then wire the live sources.

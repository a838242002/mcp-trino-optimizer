...
<task type="auto" tdd="true">
  ...
  <action>
    **Create `client.py`** at `src/mcp_trino_optimizer/adapters/trino/client.py` per D-02, D-27, D-28:
    ...
    4. On timeout: capture partial results, call `handle.cancel()`, return `TimeoutResult`
    5. Log `trino_query_executed` per D-28: `{event, request_id, query_id, statement_hash, duration_ms, result_row_count, trino_state, auth_mode}` — statement_hash is `hashlib.sha256(sql.encode()).hexdigest()`, raw SQL is NEVER logged
  ...
  <verify>
    <automated>uv run pytest tests/adapters/test_trino_client_invariant.py tests/adapters/test_query_logging.py -v -x</automated>
  </verify>
  <acceptance_criteria>

...
|---|-------|---------|---------------|
| A1 | `pydantic.BaseModel.model_json_schema(by_alias=True)` produces `additionalProperties: false` when `ConfigDict(extra="forbid")` is set | §3.2, §9 | Schema lint fails on the single Phase 1 tool; easy to fix with an explicit pydantic schema override or `json_schema_extra={"additionalProperties": False}` | **RESOLVED:** 01-04-app-tools-transports-cli-PLAN.md Task 1 documents the `json_schema_extra={"additionalProperties": False}` fallback path inside the selftest models and instructs the executor to apply it if the default `extra="forbid"` path does not emit `additionalProperties: false`. |
...
| A5 | FastMCP v1.27.0 does NOT write anything to stdout before `stdio_server()` captures the fd (e.g., in `configure_logging`) | §4.3 | If the SDK writes a banner on import, our SentinelWriter catches it but logs it as a "violation" — cosmetically wrong but not a correctness bug. Mitigation: import `mcp.server.fastmcp` AFTER calling `install_stdout_guard()` | **RESOLVED:** 01-04-app-tools-transports-cli-PLAN.md Task 2 `run_stdio()` calls `install_stdout_guard()` BEFORE any in-function FastMCP import; the task action explicitly notes that if the SDK writes on import, an `import mcp.server.fastmcp` must be added inside `run_stdio` AFTER the guard install. |
...
## 20. Risks + Open Questions for the Planner (RESOLVED — see inline annotations)
...
1. **`_tool_manager` is a private attribute.** If the SDK renames it in a minor version, `schema_lint` breaks. **Mitigation:** the schema_lint CI test fails loudly; pin `mcp>=1.27.0,<1.28` to tighten the window. Planner decides whether to pin tighter than CLAUDE.md's `<2`.
   **RESOLVED:** 01-02-safety-primitives-PLAN.md Task 3 documents the private-attribute risk in the `schema_lint.py` module docstring and the `test_schema_lint_detects_violation` test in `tests/safety/test_schema_lint.py` (stubbed by 01-01 Task 2) acts as a CI canary — any SDK rename breaks it loudly.
...
2. **FastMCP's Tool JSON Schema includes pydantic's `$defs` expansion** for nested models. The schema_lint walker in §9 handles `$defs` and `definitions` but the Phase 1 selftest is simple enough that this code path isn't exercised at phase gate time. Planner should add a test with a nested-input tool (marked `@pytest.mark.xfail` until Phase 2) to exercise the recursion.
   **RESOLVED:** 01-02-safety-primitives-PLAN.md Task 3 implements the `$defs`/`definitions`/`anyOf`/`oneOf`/`allOf` recursion in `_check_schema`; the fake-bad-tool test in `tests/safety/test_schema_lint.py` exercises the recursion path via pydantic-generated nested schemas. A dedicated nested-input `@pytest.mark.xfail` tool remains DEFERRED until Phase 2 adds a tool with a nested BaseModel input (low risk — walker logic is covered by unit tests today).
...
3. **The `http_bearer_token` in HTTP mode is transmitted in plaintext over HTTP** if TLS isn't in front of the server. CONTEXT.md D-07 notes "no TLS fields in Settings — defer to Phase 2." This is correct for scope but the planner should add **a loud WARNING log on HTTP transport startup** saying "This server binds plaintext HTTP; use a reverse proxy for TLS in production."
   **RESOLVED:** 01-04-app-tools-transports-cli-PLAN.md Task 2 — `run_streamable_http()` emits the `plaintext_http_warning` WARNING log event at startup (acceptance criterion: `grep -c "plaintext_http_warning" src/mcp_trino_optimizer/transports.py` returns 1).
...
4. **Uvicorn's default logger writes to stdout.** Even in HTTP mode (which doesn't use stdio for the protocol), uvicorn's access log to stdout is a smell if later phases share config with stdio code. The planner should pass `log_config=None` AND set `log_level="error"` to uvicorn, OR inject a structlog logger. §3.5 recommends `log_level="error"` as the minimum; stronger hardening is a Phase 9 concern.
   **RESOLVED:** 01-04-app-tools-transports-cli-PLAN.md Task 2 — `run_streamable_http()` instantiates `uvicorn.Config(..., log_level="error")`. Stronger hardening (`log_config=None` + structlog injection) is DEFERRED to Phase 9 as noted.
...
5. **The 9-cell CI matrix will cost ~10-15 minutes per run.** This is within GitHub Actions free-tier budgets but the planner should add `[skip ci]` conventions for doc-only commits, and consider `paths-ignore: ["*.md"]` on the `unit-smoke` job to avoid matrix-blowup on documentation PRs.
   **RESOLVED (partial):** 01-06-ci-precommit-PLAN.md is expected to define the `paths-ignore` / `[skip ci]` conventions. If 01-06 does not address it, this remains a DEFERRED nice-to-have — not a Phase 1 blocker because it is a CI-cost optimization, not a correctness concern.
...
6. **Does `uv tool install .` actually work from a local path on Windows?** `[ASSUMED]` — the canonical uv docs document this for published PyPI names but the local-path case on Windows is less well-tested. The CI workflow in §12 exercises it explicitly so a failure is caught at the Phase 1 gate. If it turns out to fail, the mitigation is to use `pip install .` only on Windows and still test `uv tool install` on macOS/Linux.
   **RESOLVED:** 01-06-ci-precommit-PLAN.md is the planned home for the CI matrix that exercises this install path on the Windows cell. Test outcome at the Phase 1 gate is the verification.
...
7. **`git_sha` resolution via subprocess is a security surface.** Running `git rev-parse` from server code is safe (no user input reaches the command) but the planner should ensure the subprocess cannot be influenced by `PATH` manipulation. The absolute-path lookup (`shutil.which("git")`) is a harden-step the planner may choose to add.
   **RESOLVED (partial):** 01-03-settings-logging-runtime-PLAN.md Task 1 implements the three-tier resolver with `timeout=1`, `check=False`, and exception suppression. Tier 1 (env var) and Tier 2 (baked `_git_sha.txt`) are preferred in CI and Docker, so the subprocess path is a fallback only. `shutil.which("git")` hardening is DEFERRED as a nice-to-have — not a correctness concern because the resolver never passes user input to argv.
...
8. **Phase 1's single `mcp_selftest` tool means schema_lint has very little surface area to protect.** The real value of schema_lint shows up in Phase 8 when there are 8 tools. Phase 1 should still include schema_lint because **the test that schema_lint works** must exist before Phase 8 adds tools. The planner should explicitly add a "fake bad tool" test (§9.2 second test) that would fail without schema_lint — proving the guard works before it has anything real to guard.
   **RESOLVED:** 01-02-safety-primitives-PLAN.md Task 3 requires `test_schema_lint_detects_violation` (fake-bad-tool with `sql: str` lacking Field constraints) to pass as the primary green signal. The stub is created in 01-01 Task 2 and the implementation in 01-02 Task 3 makes it green.
